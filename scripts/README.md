...
### 重要: 「下書きを開く」までしかしない (誤送信防止)

relay の Outlook ハンドラは **`.Display()` で下書き作成画面を表示する** まで
しか行いません。**`.Send()` は呼ばず、自動送信は一切しない設計です**。

最終的な「送信」操作は operator 本人が Outlook クライアントで内容を確認した
上でクリックします。これにより:

- 誤送信が起きない
- Outlook 標準の「送信前ルール / 添付確認 / 警告」も通常通り効く
- 送信メールが operator の **個人 Sent Items にちゃんと残る**
