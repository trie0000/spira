...
function renderAi(): HTMLElement {
  return el('div', {}, [
    ...
    el('h2', { style: H2 }, ['プロバイダ']),
    el('ul', { style: UL }, [
      el('li', {}, [
        el('strong', {}, ['Claude (Anthropic): ']),
        'ブラウザから直接 API を呼び出し。API キーはブラウザの localStorage に保存。本格運用ではゲートウェイ経由を推奨。',
      ]),
      el('li', {}, [
        el('strong', {}, ['社内 AI (Azure OpenAI 互換): ']),
        '社内ゲートウェイ経由。Base URL とデプロイ ID のプレフィクスを設定。',
      ]),
    ]),

    el('h2', { style: H2 }, ['データの流れ']),
    el('p', { style: P }, [
      'AI チャットを開いた瞬間に該当チケットの本文・履歴が選択プロバイダに送信されます。',
      '機密情報を扱う場合はプロバイダ・モデルの選択に注意してください。',
      'AI 設定で使用を ', el('strong', {}, ['OFF']), ' にすれば送信は発生しません。',
    ]),

    el('h2', { style: H2 }, ['設定場所']),
    el('p', { style: P }, [
      CODE('歯車 → 設定 → AI 設定'),
      ' から API キー・モデル・ベース URL を設定できます。',
    ]),
  ]);
}
...
function renderFaq(): HTMLElement {
  return el('div', {}, [
    ...
    el('p', { style: P }, [
      'A. 通常運用は ¥0 (既存の Microsoft 365 ライセンス枠内)。AI チャットを Claude API 直で使う場合のみ従量課金。',
    ]),
    ...
    el('p', { style: P }, [
      ...
      ' が選択プロバイダに送信されます。他のチケット情報や全社データは送信しません。',
    ]),
  ]);
}
