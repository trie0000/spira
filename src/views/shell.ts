...
function openHelpModal(root: HTMLElement): void {
  ...
  const teamsSyncFlow = el('div', {}, [
    ...
    el('ol', { style: 'margin:0 0 var(--s-3);padding-left:1.2em;line-height:1.8;font-size:var(--fs-sm);color:var(--ink)' }, [
      el('li', {}, ['Teams チャネルにメッセージ投稿 → PA フロー④ が起動 (統合トリガー、Graph 変更通知のみ受信)']),
      el('li', {}, ['Compose で `replyToMessageId` の空チェック → 条件で返信のみ通す (親メッセージは除外)']),
      el('li', {}, ['「メッセージ詳細を取得する」で本文・差出人・日時など完全なメッセージ内容を取得']),
      el('li', {}, ['Get メッセージの差出人が aadUser のときだけ SP「項目の作成」へ進む (Bot/Spira 投稿除外)']),
    ...
    pn('構成は ', el('strong', {}, ['5 ステップ']), ': トリガー → Compose (空チェック) → 条件 → メッセージ詳細を取得する → SP 項目の作成。',
       '★ 統合トリガーは Graph 変更通知のメタデータしか返さない (`messageId`, `replyToMessageId`, `teamId`, `channelId` のみ。本文・差出人・日時はなし) ため、',
       el('strong', {}, ['「メッセージ詳細を取得する」アクションでの 2 段階取得が必須']), '。'),
    ...
    stepCard({
      ...
      note: '★ 統合トリガー。親メッセージ + 返信 + DM がすべて入ってくるので、次の STEP 2 (条件) で「チャネル内の返信のみ」に絞り込む。',
      params: [
        { field: 'メッセージの種類', value: 'チャネル', type: 'choose', hint: 'Chat は対象外。必ず「チャネル」を選ぶ。' },
        { field: 'チーム', value: '(監視対象の Team を選択)', type: 'choose', hint: 'ドロップダウンが「利用できるコンテンツはありません」のときは、Spira 設定 → Teams チャネル管理 の teamId (GUID) を fx タブで `\'xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx\'` の形で貼り付け。' },
        { field: 'チャネル (Item - 1 / -2 / -3 ...)', value: '(監視対象のチャネルを 1 つ以上選択)', type: 'choose', hint: '★ 「+ 新しいアイテム」で複数チャネル追加可。チャネル ID は Spira 設定 → Teams チャネル管理 の channelId (`19:xxx@thread.tacv2`) を fx で貼り付けると確実。Spira 側の登録チャネルと一致させること。' },
      ],
    }),

    stepCard({
      num: 2,
      title: 'Compose (作成) — replyToMessageId の空チェック',
      connector: 'Data Operation',
      action: '作成 (Compose)',
      note: '★ 新デザイナーの条件アクションは式バリデータが弱く `empty(triggerOutputs()?[...])` を直接書くと評価エラーになる。Compose を 1 個挟んでここで評価し、後段の条件は Compose 出力を Bool として参照するだけにする。アクションを ',
      params: [
        { field: 'アクション名 (リネーム推奨)', value: 'IsParent', type: 'static', hint: 'アクションタイトル部分をダブルクリックして英字 ASCII にリネーム。後段の動的コンテンツで参照しやすくなる。' },
        { field: '入力 (fx タブで式入力)', value: "empty(triggerOutputs()?['body/value']?[0]?['replyToMessageId'])", type: 'expression', hint: '★ パス注意。実トリガー JSON は Graph 変更通知形式で `body.value[0].replyToMessageId` の階層。直下 `body.replyToMessageId` ではない。返信なら False、親なら True を返す。' },
      ],
    }),

    stepCard({
      num: 3,
      title: '条件 — 返信メッセージのみ通す (親メッセージを除外)',
      connector: 'Control',
      action: '条件 (Condition)',
      note: 'STEP 2 (IsParent) の出力が false (= 返信である) のときだけ「はい」ブランチへ。新デザイナーは行 (Row) UI のみで式直貼り不可。Bot/Spira 投稿の除外は STEP 4 の後でやる (差出人情報がまだ取れていないため)。',
      params: [
        { field: '値 1', value: 'IsParent (動的コンテンツから Outputs を選択)', type: 'dynamic', hint: 'STEP 2 のアクション名がそのまま動的コンテンツの候補に出る。Outputs をクリック。' },
        { field: '演算子', value: '次の値に等しい (is equal to)', type: 'choose' },
        { field: '値 2', value: 'false', type: 'static', hint: 'ブール値の false。クォート無し直接入力。' },
      ],
    }),

    stepCard({
      num: 4,
      title: 'メッセージ詳細を取得する (Get message details) — 本文・差出人・日時を取得',
      connector: 'Microsoft Teams',
      action: 'メッセージ詳細を取得する (Get message details)',
      note: '★ STEP 3 の「はい」ブランチ内に配置。トリガーは Graph 変更通知のメタデータしか持っていないので、ここで完全なメッセージオブジェクトを取りに行く。アクション名を `GetMessage` のように ASCII にリネームしておくと後段が楽。',
      params: [
        { field: 'アクション名 (リネーム推奨)', value: 'GetMessage', type: 'static' },
        { field: 'メッセージ ID (Message id)', value: "triggerOutputs()?['body/value']?[0]?['messageId']", type: 'expression' },
        ...
        { field: '親メッセージ ID (Parent message id)', value: "triggerOutputs()?['body/value']?[0]?['replyToMessageId']", type: 'expression', hint: '返信を取得するために必須。' },
      ],
    }),
    ...
    stepCard({
      ...
      connector: 'SharePoint',
      action: '項目の作成 (Create item)',
      note: '★ STEP 3 の「はい」ブランチ内、STEP 4 の直後に配置。下記の式は GetMessage アクションの出力 (= 完全なメッセージオブジェクト) を参照する。`body(\'GetMessage\')?[...]` が動かない環境では `outputs(\'GetMessage\')?[\'body/...\']` も試す。動的コンテンツ欄に GetMessage の各フィールド (Display name, Content, Created date time 等) がリストアップされるので、ラクなのはそちらをクリックする方法。',
      params: [
        { field: 'Site Address', value: 'https://<tenant>.sharepoint.com/sites/<site>', type: 'choose' },
        { field: 'List Name', value: 'InboxMails', type: 'choose' },
        { field: 'Title', value: "concat('[Teams] ', coalesce(body('GetMessage')?['from']?['user']?['displayName'], '(不明)'))", type: 'expression', hint: 'SP 必須列。GetMessage の Display name を動的コンテンツで選んでも可。' },
        ...
        { field: 'BodyHtml', value: "body('GetMessage')?['body']?['content']", type: 'expression', hint: 'GetMessage の動的コンテンツ「Content」をクリックでも可。Spira が isHtml=true で取り込む。' },
        ...
        { field: 'FromEmail', value: "''", type: 'expression', hint: 'Teams 返信から email は直接取れない。空文字を入れておけば、Spira 側で fromName から AD ルックアップして補完される。' },
        ...
        { field: 'ConversationId', value: "concat('teams-', triggerOutputs()?['body/value']?[0]?['replyToMessageId'])", type: 'expression', hint: '★ 必須。"teams-" プレフィクス + 親メッセージ ID。トリガー側の値を使うのが軽い (GetMessage を経由しなくても取れる)。' },
        { field: 'InternetMessageId', value: "triggerOutputs()?['body/value']?[0]?['messageId']", type: 'expression', hint: '重複防止用。これもトリガー側で OK。' },
    ...
    pn('★ 任意の追加フィルタ — Bot / 特定アカウントを除外したい場合は STEP 5 の前にもう 1 個「条件」を挟み、',
       code("body('GetMessage')?['from']?['user']?['userIdentityType']"),
       ' が ', code('aadUser'), ' に等しいことをチェックする。'),
    ...
    el('div', {
      ...
      el('strong', {}, ['★ なぜ Compose と GetMessage を挟むのか']),
      el('br', {}),
  ...
  "body": {
    "value": [{
      ...
      el('strong', {}, ['→ 本文を見るには「メッセージ詳細を取得する」アクションで Graph に再問い合わせが必要']),
      '。これは Teams 標準コネクタの無料アクションなのでプレミアム連携不要。',
      el('br', {}),
      el('br', {}),
      el('strong', {}, ['★ fx タブの使い方']),
      el('br', {}),
      ...
      el('br', {}),
      '・トリガーの値: ', code("triggerOutputs()?['body/value']?[0]?['messageId']"), ' のように `body/value/0/...` 階層',
      el('br', {}),
      '・GetMessage の値: ', code("body('GetMessage')?['from']?['user']?['displayName']"), ' のように `body(\'アクション名\')` で参照 (動的コンテンツでもクリック可)',
      el('br', {}),
      el('br', {}),
    ...
    el('ul', { style: 'margin:0;padding-left:1.2em;line-height:1.8;font-size:var(--fs-sm);color:var(--ink)' }, [
      ...
      el('li', {}, [
        el('strong', {}, ['IsParent (Compose) が常に True を返す → 取り込まれない']),
        ': トリガー出力の JSON 階層が想定と違う可能性。実行履歴 → トリガー →「未加工出力の表示」で実際のパスを確認。',
        code("body/value[0]/replyToMessageId"),
        ' でなく ',
        code('body/replyToMessageId'),
        ' 等の直下に来ているケースもあるので、その場合は Compose 式の `body/value/0/` 部分を削除して直下参照に。',
      ]),
      el('li', {}, [
        el('strong', {}, ['Spira からの自動投稿 (PA フロー②) が自分自身を起こす']),
        ': フロー②の投稿は「親メッセージ」なので IsParent = True → 条件 No ブランチ送りで無視されるはず。誤発火する場合は STEP 3 の値 2 が ',
        code('false'),
        ' (真偽値、クォート無し) になっているか、文字列 ',
        code("'false'"),
        ' になっていないか確認。',
      ]),
      el('li', {}, [
        el('strong', {}, ['Bot やワークフロー由来の投稿を除外したい']),
        ': STEP 5 の前にもう 1 個「条件」を挟み、',
        code("body('GetMessage')?['from']?['user']?['userIdentityType']"),
        ' = ',
        code('aadUser'),
        ' で絞り込む。特定アカウント除外なら userPrincipalName で「次の値を含まない」を AND で追加。',
      ]),
      el('li', {}, [
        el('strong', {}, ['親メッセージや DM が誤って取り込まれる']),
        ': STEP 1 のメッセージの種類が「チャネル」になっているか、STEP 2 の式が ',
        code('body/value/0/replyToMessageId'),
        ' を見ているか、STEP 3 の値 2 が ',
        code('false'),
        ' (真偽値) になっているか確認。',
      ]),
      el('li', {}, [
      ...
      el('li', {}, [
        ...
        ' のように直書き (アクション名は STEP 4 で設定したリネーム後の名前を使う)。',
      ]),
      el('li', {}, [
        el('strong', {}, ['チャネル選択で「利用できるコンテンツはありません」と出る']),
        ': チーム選択を一度別に切り替えて戻す → ページ再読み込み → それでもダメなら ',
        el('strong', {}, ['fx タブ']),
        ' で Spira 設定 → Teams チャネル管理 の channelId (',
