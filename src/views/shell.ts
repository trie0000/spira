...
function openHelpModal(root: HTMLElement): void {
  ...
  const teamsSyncFlow = el('div', {}, [
    ...
    pn('構成は ', el('strong', {}, ['3 ステップ']), ': トリガー → 条件 → SP 項目の作成。'),
    ...
    stepCard({
      ...
      connector: 'Control',
      action: '条件 (Condition)',
      note: '統合トリガーは親メッセージでも発火するので、ここで「replyToId が非空 (= 返信) かつ aadUser (= Bot/Spira 投稿ではない)」のときだけ Yes ブランチへ進める。No ブランチは空のままで OK (= フロー終了)。新デザイナーの条件アクションは式直貼り不可なので、行 (Row) を 2 つ作って AND で連結する。',
      params: [
        { field: '行 1 / 値 1 (fx タブで式入力)', value: "empty(triggerOutputs()?['body/replyToId'])", type: 'expression', hint: '「empty」関数で replyToId が空かをチェック。fx タブから入力。' },
        { field: '行 1 / 演算子', value: '次の値に等しい (is equal to)', type: 'choose' },
        { field: '行 1 / 値 2', value: 'false', type: 'static', hint: 'ブール値の false。クォート無しで直接入力 (文字列ではなく真偽値)。' },
        { field: '行 2 / 値 1 (fx タブで式入力)', value: "triggerOutputs()?['body/from/user/userIdentityType']", type: 'expression' },
        { field: '行 2 / 演算子', value: '次の値に等しい (is equal to)', type: 'choose' },
        { field: '行 2 / 値 2', value: 'aadUser', type: 'static', hint: '文字列。クォート不要で直接入力。' },
        { field: '行同士の結合', value: 'すべての条件 (And)', type: 'choose', hint: '行の上または左にあるトグル。デフォルトで And になっていることが多い。' },
      ],
    }),
    ...
    stepCard({
      ...
      connector: 'SharePoint',
      action: '項目の作成 (Create item)',
      note: '★ このアクションは STEP 2 の条件の「はい (True)」ブランチの中に配置すること (「いいえ」ブランチは空のまま)。★★ 統合トリガーは動的コンテンツ欄に主要フィールドを出さないことが多い。下記の値は基本すべて「fx タブ (式)」から直接貼り付けて入力する。Spira は ConversationId が "teams-..." で始まる行を Teams 返信と認識し、parentMessageId 部分を Tickets の InternalThreadId / UserThreadId と照合して自動紐付けする。',
      params: [
        { field: 'Site Address', value: 'https://<tenant>.sharepoint.com/sites/<site>', type: 'choose' },
        { field: 'List Name', value: 'InboxMails', type: 'choose' },
        { field: 'Title', value: "concat('[Teams] ', coalesce(triggerOutputs()?['body/from/user/displayName'], '(不明)'))", type: 'expression', hint: 'SP 必須列。識別用。fx タブで貼り付け。' },
        { field: 'Subject', value: "concat('[Teams] ', coalesce(triggerOutputs()?['body/from/user/displayName'], '(不明)'), ': ', substring(coalesce(triggerOutputs()?['body/body/plainTextContent'], ''), 0, min(60, length(coalesce(triggerOutputs()?['body/body/plainTextContent'], '')))))", type: 'expression', hint: '受信一覧での表示用。冒頭 60 字。★ plainTextContent が null の返信があるので必ず coalesce() で空文字フォールバックすること。' },
        { field: 'BodyHtml', value: "triggerOutputs()?['body/body/content']", type: 'expression', hint: 'Teams 返信の HTML 本文。Spira が isHtml=true で取り込む。動的コンテンツに「Message body content」があればそれをクリックでも可。' },
        { field: 'BodyText', value: "coalesce(triggerOutputs()?['body/body/plainTextContent'], '')", type: 'expression', hint: 'plainTextContent が null の場合があるので coalesce() で空文字に。SP 側で Null 拒否されるのを防ぐ。' },
        { field: 'FromName', value: "triggerOutputs()?['body/from/user/displayName']", type: 'expression' },
        { field: 'FromEmail', value: "''", type: 'expression', hint: 'Teams 返信から email は直接取れない (Graph API なしのため)。fx で空文字 ` \'\' ` を入れておけば、Spira 側で fromName から AD ルックアップして補完される。' },
        { field: 'ReceivedAt', value: "triggerOutputs()?['body/createdDateTime']", type: 'expression' },
        { field: 'SentAt', value: "triggerOutputs()?['body/createdDateTime']", type: 'expression' },
        ...
        { field: 'InternetMessageId', value: "triggerOutputs()?['body/id']", type: 'expression', hint: '重複防止用に Teams メッセージ ID を入れる。' },
    ...
    el('div', {
      ...
      el('strong', {}, ['★ fx タブの使い方']),
      el('br', {}),
      'PA で各入力欄をクリック → 右ペインに「動的コンテンツ」「式 (fx)」のタブが出る → ',
      el('strong', {}, ['「式 (fx)」タブを選択']),
      ' → 下の入力欄に上記の `triggerOutputs()?[...]` 形式の文字列を ',
      el('strong', {}, ['そのまま貼り付け']),
      ' → 「追加」ボタン (もしくは Enter)。',
      el('br', {}),
      '動的コンテンツに当該フィールドが出ていなくても、トリガーの生 JSON に存在すれば fx 経由で確実に取れる (動的コンテンツはあくまで「PA が UI に出してくれる候補リスト」であり、コネクタ世代によって出る/出ないがバラバラ)。',
      el('br', {}),
      el('strong', {}, ['実トリガー出力を確認したいとき:']),
      ' 仮で何でもいいから値を入れて保存 → テスト返信 1 件 → PA「実行履歴」→ 該当ラン → STEP 1 をクリック → ',
      el('strong', {}, ['「未加工出力の表示」']),
      ' で JSON 全体を確認。`body.replyToId` / `body.from.user.displayName` / `body.body.plainTextContent` / `body.body.content` / `body.createdDateTime` / `body.id` が実際にどんなキーで来ているか目視で確認できる。',
    ]),
    ...
    el('ul', { style: 'margin:0;padding-left:1.2em;line-height:1.8;font-size:var(--fs-sm);color:var(--ink)' }, [
      ...
      el('li', {}, [
        ...
        ': PA フロー②の投稿は「親メッセージ」なので、STEP 2 の行 1「',
        code("empty(...replyToId) = false"),
        '」が成立せず No ブランチ送り (= 無視) されるはず。誤発火する場合は STEP 2 の行 1 値 2 が ',
        code('false'),
      ...
      el('li', {}, [
        ...
        ': 既に STEP 2 の行 2 で ',
        code("userIdentityType = 'aadUser'"),
        ' を入れて aadUser のみ通している。さらに絞りたい場合 (例: 特定アカウントを除外) は ',
        el('strong', {}, ['+ 新しい行を追加']),
        ' で行 3 を足し、値 1 = ',
        code("triggerOutputs()?['body/from/user/userPrincipalName']"),
        '、演算子 = ',
        code('次の値を含まない'),
        '、値 2 = `bot@...` のように設定。',
      ]),
      el('li', {}, [
        ...
        ': STEP 1 のメッセージの種類が「チャネル」になっているか、STEP 2 の行 1 (replyToId が非空チェック) が入っているか、行同士の結合が「すべての条件 (And)」になっているか確認。3 つそろっていれば親・DM は除外される。',
      ]),
      el('li', {}, [
        el('strong', {}, ['動的コンテンツにメッセージ本文や displayName 等が出てこない']),
        ': 統合トリガー (When a new message is added to a chat or channel) の仕様上、動的コンテンツ欄には主要フィールドしか出てこない (`Message body content` くらい)。',
        el('strong', {}, ['解決策: 各 SP 列の入力欄で「fx タブ」を選び、上の STEP 3 の表に書いてある ']),
        code("triggerOutputs()?['body/...']"),
        el('strong', {}, [' 式を直接貼り付ける']),
        '。動的コンテンツに無くてもトリガーの生 JSON に存在するので fx 経由で必ず取れる。',
        el('br', {}),
        '実 JSON 構造を確認したい場合は、フローを一度実行 → 実行履歴 → STEP 1 →「未加工出力の表示」で `body.replyToId` `body.from.user.displayName` `body.body.plainTextContent` 等のパスを目視確認 → 違ってたら式の `body/...` 部分を実物に差し替え。',
      ]),
      el('li', {}, [
