...
import { listTicketsMock, listSiteUsersMock } from '../api/mock';
import { ticketStatusList, priorityList } from '../api/sp';
import { getState, setFilter, setState } from '../state';
import type { Ticket, TicketStatus, Priority } from '../types';
import { fmtDate, isOverdue, initials } from '../utils/dom';

export function renderTicketList(): HTMLElement {
...
function renderToolbar(): HTMLElement {
  ...
  statusSel.addEventListener('change', () => setFilter({ status: (statusSel as HTMLSelectElement).value }));
  ...
  assigneeSel.addEventListener('change', () => setFilter({ assignee: (assigneeSel as HTMLSelectElement).value }));
  ...
  prioSel.addEventListener('change', () => setFilter({ priority: (prioSel as HTMLSelectElement).value }));
  ...
  sortSel.addEventListener('change', () => setState({ sortBy: (sortSel as HTMLSelectElement).value as 'updated' | 'priority' | 'due' }));
  ...
  const searchWrap = el('div', { class: 'spira-search-wrap' }, [
  ...
  if (rows.length === 0) {
    return el('div', { class: 'spira-content' }, [
      el('div', { class: 'spira-empty' }, [
        el('div', { class: 'spira-empty-title' }, ['チケットはありません']),
